    ...
    public GameField(long height, long width, Stage Game,Group group) throws FileNotFoundException
    {
                        ...
                        if (step == 1)
                        {
                            ...
                            for (i = 0;i < TowersCoordinate.size();i++)
                            {
                                if (x >= TowersCoordinate.get(i).getX() && x <= TowersCoordinate.get(i).getX() + Config.TILE&& y >= TowersCoordinate.get(i).getY() && x <= TowersCoordinate.get(i).getY() + Config.TILE && TowersCoordinate.get(i).checkPlaceTower() == true && TowersCoordinate.size() > 0)
                                {
                                    ...
                                    TowerSouls.add(new MachineGunTower(FieldGroup,x,y,0,0,0,0,100,100));
        ...
        for (i = 0;i < SmallerEnemies.size();i++)
        {
            ...
            Keys[i] = new KeyFrame(Duration.millis(i * Config.Tick + Config.PreparingTime),event ->
            {
               ...
               // enemy.HPBar.move(Config.PathforMap())
                //enemy.ShowHP();
            });
        ...
        AnimationTimer gameLoop = new AnimationTimer()
        {
            public void handle(long now)
            {
                ...
                for (Enemy SurvivedEnemies: Enemies)
                {
                    if (SurvivedEnemies.getImageView().getTranslateX() == Config.VanishX && SurvivedEnemies.getImageView().getTranslateY() == Config.VanishY)
                    {
                        PlayerHealth -= 50;
                    }
                    if (Enemies.size() > 73 || PlayerHealth <= 0)
                    {
                        Image GameOverImage = new Image("GameFiles/Image/Game Over Image.png");
                        ImageView ViewGameOverImage = new ImageView(GameOverImage);
                        ViewGameOverImage.setX(0);
                        ViewGameOverImage.setY(0);
                        ViewGameOverImage.setFitWidth(Config.WIDTH);
                        ViewGameOverImage.setFitHeight(Config.HEIGHT);
                        FieldGroup.getChildren().add(ViewGameOverImage);
                    }
                }
                // movement
